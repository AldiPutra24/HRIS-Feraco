...
- **Tahap 3 — Complete Onboarding → Employee + Account creation**:
  - `models.py`: `Onboarding` gained `employee` (OneToOne → `personnel.Employee`, SET_NULL, related_name `onboarding_entry`) + `completed_by` (FK AUTH_USER_MODEL, related_name `completed_onboardings`). Migration `0003_onboarding_completed_by_onboarding_employee`.
  - `TRANSITIONS` hardened: `READY` only allows `{'CANCELLED'}` (removed `COMPLETED`); `COMPLETED` terminal (no outgoing). `COMPLETED` reachable ONLY via `complete` action — never via PATCH status or `transition` action.
  - `services.py` `complete_onboarding(onboarding, request)` (`@transaction.atomic`): guards status==COMPLETED → idempotent return; status!=READY → ValidationError. Reads `OnboardingData`, creates `Employee` (employee_id `EMP-{year}-{seq:04d}` via Max aggregate, full_name/NIK/birthplace+date/address/phone/emergency contact/bank/NPWP/BPJS/department/position/reporting_to/join_date/employment_type), `EmployeeContract` (contract_number `CTR-{employee_id}`, PKWTT no end_date, PKWT +1yr end_date, probation only PKWTT), `User` account (username from personal_email or full_name, EMPLOYEE role, random password via `get_random_string(16)` — Django 5.2 removed `make_random_password`), links `employee.user`, sets onboarding COMPLETED/completed_at/completed_by/employee, writes `OnboardingStatusHistory` READY→COMPLETED, logs 4 audit events (employee create / contract create / user create / onboarding complete). Password never exposed via API/log/DB.
  - `serializers.py`: `OnboardingSerializer` adds `completed_by_name`, `employee_id`/`employee_name`/`employee_status` (source employee.*), `account_status` (`NO_ACCOUNT`/`ACTIVE`/`INACTIVE`).
  - `views.py`: new `POST /api/onboarding/{id}/complete/` action (READY→200, COMPLETED→200 idempotent, else 400; catches ValidationError→400). `partial_update` blocks `status=='COMPLETED'` (400). `transition` action blocks `to_status=='COMPLETED'` (400).
  - `tests.py`: `CompleteOnboardingTests` (9 tests) — creates employee+contract+account, requires READY, fails without readiness, idempotent, RBAC, PATCH COMPLETED blocked, transition-to-COMPLETED blocked, audit logged, exposes employee info. Fixed 10 pre-existing failures (test_valid_forward_transitions + test_completed_is_terminal across 5 classes) to use `/complete/` instead of transition-to-COMPLETED.
  - **Full onboarding suite: 94 tests OK.** Full backend suite in progress.
- **Tahap 2 models** (`onboarding/models.py`): `OnboardingData` (OTTO→Onboarding, 17 fields: NIK/nama/tempat tanggal lahir/gender/agama/alamat/telepon/kontak darurat/no rekening/nama rekening/NPWP/BPJS Kesehatan/BPJS Ketenagakerjaan/tanggal bergabung/jenis pekerjaan), `OnboardingChecklistItem` (OTTO→Onboarding, category/name/ordering/required/completed/completed_by/completed_at), `OnboardingDocument` (OTTO→Onboarding, document_type/pilihan, file/upload via Supabase, status PENDING/APPROVED/REJECTED + rejection_reason, uploaded_by/at). All with soft-delete (is_active, deleted_at).
- **Tahap 1** (base): Bridge Candidate (`OFFER_ACCEPTED`) → future Employee conversion. `Onboarding` model + `OnboardingStatusHistory` + base ViewSet/Serializer/Tests (12).
- **Tahap 2 models** (`onboarding/models.py`): `OnboardingData` (OTTO→Onboarding, 17 fields: NIK/nama/tempat tanggal lahir/gender/agama/alamat/telepon/kontak darurat/no rekening/nama rekening/NPWP/BPJS Kesehatan/BPJS Ketenagakerjaan/tanggal bergabung/jenis pekerjaan), `OnboardingChecklistItem` (OTTO→Onboarding, category/name/ordering/required/completed/completed_by/completed_at), `OnboardingDocument` (OTTO→Onboarding, document_type/pilihan, file/upload via Supabase, status PENDING/APPROVED/REJECTED + rejection_reason, uploaded_by/at). All with soft-delete (is_active, deleted_at).
- **Tahap 2 serializers** (`serializers.py`): All 3 CRUD serializers; `OnboardingChecklistItemSerializer.update()` auto-sets `completed_at`/`completed_by` when `completed=True`, clears when False.
- **Tahap 2 services** (`services.py`): `DEFAULT_CHECKLIST` (9 items across 3 categories: Dokumen Pribadi/Administrasi Perusahaan/Verifikasi), `MANDATORY_DATA_FIELDS` (18 fields), `MANDATORY_DOCUMENT_TYPES` (5 types), `readiness_errors(onboarding)` → list of blockers (missing data, incomplete mandatory checklist items, missing mandatory docs, candidate not on OFFER_ACCEPTED, terminal status).
- **Tahap 2 views** (`views.py`): `OnboardingDataViewSet` (CRUD), `OnboardingChecklistItemViewSet` (CRUD, create_default on first GET if empty), `OnboardingDocumentViewSet` (CRUD + upload to Supabase Storage + download signed URL). `OnboardingViewSet` extended with `data`/`checklist`/`documents`/`readiness` actions. Readiness endpoint returns `{ready, progress, errors, status}`.
- **Tahap 2 tests**: 73 tests (pass all) — CRUD each model, upload/download document, readiness blocked by missing data/checklist/docs, readiness passes when all met, RBAC, Supabase offline fallback, `create_default_checklist` on first GET.
- **Tahap 2 frontend**: `src/lib/onboarding.ts` extended with `OnboardingData`/`OnboardingChecklistItem`/`OnboardingDocument`/`OnboardingReadiness` types + all API functions (get/update data, list/update checklist, list/upload/update/delete/download documents, get readiness). `onboarding-detail-page.tsx` rewritten with 4 tabs: Data (editable grid 17 fields, save button, HR edit / management read-only), Checklist (grouped by category, toggle circleCheck/circle, optional note, required badge, completed_by info), Documents (upload form with doc type select + file input + upload button, table with download/approve/reject/delete actions, status badges), Summary (readiness card with progress bar + error list + "Siap" indicator, status history table).
- **Full backend validation**: 256 tests pass (73 onboarding + 34 recruitment + 27 leaves + 23 leaves business rules + 66 personnel/accounts/audit + 33 others). `manage.py check` OK.
- **Full frontend validation**: tsc 0 errors, oxlint 0 warnings (new files), `next build` compiled successfully.
- Routes unchanged: `/dashboard/recruitment/onboarding` + `/dashboard/recruitment/onboarding/[id]`.
