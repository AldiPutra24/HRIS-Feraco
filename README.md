...
**Reimbursement Module & Recruitment Module** — last updated 2026-08-31

- **Recruitment Module (`apps.recruitment`)** — Job management + public job portal. HR page `/dashboard/recruitment/jobs` (list/search/filter, Add/Edit, Open/Close/Reopen, Delete DRAFT-only, Copy Link); public portal `/jobs/{slug}` (no auth, apply form — name/email/phone). Status is **system-managed by backend**: complete required fields → `OPEN`, incomplete → `DRAFT`; `DRAFT` editable + hidden, auto-`OPEN` when completed, public only `OPEN` (excludes expired close_date); frontend cannot force status. RBAC `RECRUITMENT_ADMIN_ROLES` = ADMIN/HR_STAFF/HR_LEAD; applications/candidates + CV upload to Supabase Storage bucket `recruitment-cvs`; audit via existing `log_event`. 23 tests. Deployed to prod.
...
- **Validation** — Backend `check` & 108 unit tests pass (23 recruitment); Frontend `tsc`, `oxlint`, and `next build` clean.