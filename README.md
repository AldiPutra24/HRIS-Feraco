...
- **GENERAL_MANAGER** = read-only: semua GET diperbolehkan, seluruh mutation (calculate/approve/paid/lock, struktur gaji, payment type, tax config, manual item) 403 di backend. GM juga kini bisa download payslip miliknya sendiri.
- **HR_STAFF** = tidak punya akses payroll sama sekali: dihapus dari `PAYROLL_ADMIN_ROLES`/`PAYROLL_VIEW_ROLES`, semua endpoint payroll 403, menu Payroll disembunyikan dari sidebar (gated `access: { roles: ['admin', 'hr_lead'] }`).
- `SalaryStructurePermission` kini write-only untuk ADMIN/HR_LEAD; role lain tetap self-service baca struktur sendiri. MANAGEMENT/EMPLOYEE self-service tidak berubah. Test: `PayrollRoleAccessTests` (HR Lead full + transitions, GM read-only 403 mutations, HR_STAFF 403). Suite payroll 136 tests OK; `tsc` + `next build` clean.
