...
**Access Control: Payroll per Role (HR_LEAD / HR_STAFF / GM)** — last updated 2026-09-25

- **HR_LEAD** = full payroll access (payment types, struktur gaji, processing, konfigurasi pajak, periode, payslip, recap/export, semua transition Calculate/Review/Approve/Paid/Lock) — setara ADMIN di modul payroll.
- **GENERAL_MANAGER** = view-only console: bisa membuka dan melihat Payment Types, Struktur Gaji, Payroll Processing, dan Konfigurasi Pajak (bukan self-service slip gaji), tapi semua tombol mutation disembunyikan (frontend `readOnly`) dan backend menolak setiap mutation dengan 403 (calculate/approve/paid/lock, create/update structure/component/tax, manual item, recap pun tetap hanya untuk ADMIN/HR_LEAD).
- **HR_STAFF** = tanpa akses payroll admin/konfigurasi (di luar `PAYROLL_ADMIN_ROLES`; semua endpoint admin 403), tapi tetap punya **self-service**: `my-payslips`, download payslip sendiri, dan struktur gaji miliknya — menu Payroll disembunyikan dari sidebar (gated `access: { roles: ['admin', 'hr_lead', 'general_manager'] }`); HR Staff memakai menu Slip Gaji employee (`/dashboard/employee/payslip`).
- `SalaryStructurePermission` write-only untuk ADMIN/HR_LEAD; role lain self-service baca struktur sendiri. MANAGEMENT/EMPLOYEE self-service tidak berubah. Frontend `PayrollPage` menerima prop `readOnly` (GM) yang menyembunyikan semua tombol tambah/edit/hapus/transition/manual item dan menonaktifkan input konfigurasi pajak. Test: `PayrollRoleAccessTests` (HR Lead full + transitions, GM read-only 403 semua mutation, HR_STAFF self-service-only). Suite payroll 48 role/scope tests OK; `tsc` + `next build` clean; verifikasi visual HR Lead & GM di browser.

**Bug Fix: Freelancer hasil mapping Candidate tidak muncul di list default** — last updated 2026-09-24

- Root cause: `FreelancerViewSet` memakai pagination default DRF (`PAGE_SIZE=20`) tanpa `pagination_class`, sementara frontend `listFreelancers` hanya meng-unwrap page 1. Ketika pool melebihi 20 record, freelancer baru (termasuk hasil "Pindahkan ke Freelance / Talent Pool") jatuh ke halaman 2 dan hanya terlihat lewat search. Fix: `pagination_class` `LargePagePagination` dengan `?page_size=` (pola sama dengan personnel), dan `listFreelancers` mengirim `page_size=1000` default. Tanpa `page_size`, endpoint tetap terpaginasi normal (page 1 = 20). 4 regression test baru (`FreelancerInitialListPaginationTests`); suite freelance 59 OK; `tsc` clean, `next build` OK.
